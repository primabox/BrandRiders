...
if (!section1 || !section2 || !section3 || !section4) {
  console.warn('Mobile4: One or more sections not found');
}

// Get SVG elements
const svg = document.getElementById('mobile4Svg');
const path = document.getElementById('mobile4Path');
const car = document.getElementById('mobile4Car');

if (!svg || !path || !car) {
  console.warn('Mobile4: SVG elements not found');
}
...
// Configuration
const CONFIG = {
  startOffset: 0.75,  // Start at 75% of combined scroll (earlier)
  endOffset: 0.85,    // End at 85% of combined scroll (faster)
  pathStart: 1,       // Reversed: start from end
  pathEnd: 0          // Reversed: end at start
};

function updateCar() {
  if (!section1 || !section2 || !section3 || !section4 || !svg || !path || !car) return;
  ...
  const topSection = Math.min(rect1.top, rect2.top, rect3.top, rect4.top);
  const bottomSection = Math.max(rect1.bottom, rect2.bottom, rect3.bottom, rect4.bottom);
